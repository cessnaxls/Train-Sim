...
import './styles.css';
import {World3D,TrainPreview3D} from './ThreeWorld.jsx';
...
function App(){
  ...
  const[train,setTrain]=useState({name:'WRX-8',kind:'electric',powerKW:5200,maxTEKN:320,maxSpeed:160,locoMassT:88,carCount:6,carMassT:42,seatsPerCar:76,standingPerCar:34,paxLoad:.78,serviceBrakeKN:430,emergencyBrakeKN:650,regenPct:55,davisA:1.6,davisB:.025,davisC:.00055,adhesion:.29,bodyColor:'#d6aa43',stripeColor:'#3de2b1'});
...
function TrainDesigner({train,setTrain,paxCapacity,pax,paxMassT,tareMassT,grossMassT,trainLengthM,metrics,onBack,onDrive}){
  const set=(k,v)=>setTrain(t=>({...t,[k]:v}));
  return <main className="train-layout"><section className="train-stage three-stage"><TrainPreview3D train={train}/><div className="identity floating"><input value={train.name} onChange={e=>set('name',e.target.value)}/><span>{train.kind.toUpperCase()} • {train.carCount+1}-UNIT CONSIST • LIVE 3D</span></div><div className="bigmetrics"><div><small>Gross mass</small><strong>{grossMassT.toFixed(1)} t</strong></div><div><small>Passengers</small><strong>{pax}</strong><span>/ {paxCapacity}</span></div><div><small>Length</small><strong>{trainLengthM.toFixed(0)} m</strong></div><div><small>Route</small><strong>{fmtDist(metrics.distance)}</strong></div></div></section>
    <aside className="panel train-panel"><div className="paneltitle"><button onClick={onBack}>←</button><div><h1>Train engineering</h1><p>Build the physical consist and watch it update in 3D.</p></div></div><div className="seg"><button className={train.kind==='electric'?'on':''} onClick={()=>set('kind','electric')}>⚡ Electric</button><button className={train.kind==='diesel'?'on':''} onClick={()=>set('kind','diesel')}>◈ Diesel</button></div>
      <h3>Livery</h3><div className="color-fields"><label>Body <input type="color" value={train.bodyColor} onChange={e=>set('bodyColor',e.target.value)}/></label><label>Stripe <input type="color" value={train.stripeColor} onChange={e=>set('stripeColor',e.target.value)}/></label></div>
      ...
      <div className="massbox"><span>Tare <b>{tareMassT.toFixed(1)} t</b></span><span>Passenger mass <b>{paxMassT.toFixed(1)} t</b></span><span>Gross <b>{grossMassT.toFixed(1)} t</b></span></div><button className="primary" onClick={onDrive}>Enter 3D cab →</button>
    </aside></main>
}
...
function DriveSim({points,stations,terrain,train,grossMassT,pax,mode,onExit}){
  ...
  const[state,setState]=useState({x:0,v:0,t:0,throttle:0,brake:0,energy:0,score:1000,stopped:[],emergency:false,overspeedTime:0}),[paused,setPaused]=useState(false),[cameraMode,setCameraMode]=useState('cab'),pauseRef=useRef(false);useEffect(()=>{pauseRef.current=paused},[paused]);
  ...
  const cycleCamera=()=>setCameraMode(m=>m==='cab'?'chase':m==='chase'?'drone':'cab');
  return <main className={`cab three-cab ${mode}`}><World3D route={points} terrain={terrain} stations={stations} train={train} state={state} mode={mode} cameraMode={cameraMode}/><div className="top-hud"><button onClick={onExit}>‹ ENGINEERING</button><div><span>{train.name} • {cameraMode.toUpperCase()}</span><b>{fmtTime(state.t)}</b></div><div className="hud-buttons"><button onClick={cycleCamera}>◉ CAMERA</button><button onClick={()=>setPaused(p=>!p)}>{paused?'▶ RESUME':'Ⅱ PAUSE'}</button></div></div>
